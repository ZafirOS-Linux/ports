...
#echo 'rust rust-bin' | as_root tee -a $ROOTFS/etc/zfrpkg.alias

if [ "$INIT" = runit ]; then
	echo 'rc runit-rc' | as_root tee -a $ROOTFS/etc/zfrpkg.alias
	as_root $ROOTFS/usr/bin/xchroot $ROOTFS zfr remove -y sysvinit rc
	pkgs="$pkgs,runit-rc"
fi
